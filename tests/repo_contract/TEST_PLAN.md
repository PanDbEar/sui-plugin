...
| **RC3** | Canonical Source Surface | The `src/` directory contains exactly the canonical 6 files: `Core.cpp`, `Core.hpp`, `main.cpp`, `Natives.cpp`, `Natives.hpp`, `Utils.hpp`. No prototype, legacy wrapper, or dead code files exist. | Automated Static Checker (`check_source_surface.py`) | Exactly 6 files match canonical set. |

...
- **SUI-018**: In-flight callback execution error leaves partial external UI resources in indeterminate state. Status: `MITIGATED — callback-failure quarantine and best-effort compensation implemented; raw untracked host resources remain outside SUI ownership`.
  ...
  - On `cbCreate` failure (AMX runtime error or capacity accounting failure), SUI refrains from marking group created (`isCreated = false`), debits zero textdraw capacity (`activeTextDrawCount` preserved), sets `recoveryDestroyRequired = true`, and attempts an immediate compensating `cbDestroy` callback.
  ...
  - Added permanent runtime regression test suite 13 `tests/callback_error_recovery/` (scenarios F1–F12, 12 assertions), verifying real NPC PlayerTextDraw handle reuse (zero slot creep across repeated failure/recovery cycles).
  - Verified 177 / 177 assertions across all 13 permanent test suites, 23 / 23 Pawn scripts compile cleanly (0 errors, 0 warnings), 7 / 7 API contract checks, 6 / 6 repository contract checks, 12 / 12 release contract checks, and package deployment smoke test PASS.

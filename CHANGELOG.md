...
- **SUI-010**: Public Pawn API Synchronization, Return Contract Truthfulness, Include/Documentation Alignment, and Example Compatibility. Status: `FIXED — public API synchronization verified`.
  ...
  - Hardened `SUI_RegisterGroup` stock helper following the Prevalidated Truthful Helper model (Model B): parameter prevalidation strictly rejects negative sizes, negative timeouts, or out-of-bounds priorities before invoking registration native; removed destructive `SUI_DestroyGroup` call from the error path to eliminate the hazard of inadvertently destroying live UI groups upon failed re-registration. Return contract is truthful: `1` indicates complete setup success, `0` indicates any validation, registration, or configuration failure.
  ...
  - Verified live execution of all 11 permanent test suites on headless 32-bit Linux SA-MP dedicated server (`samp03svr`) with cumulative 150 / 150 PASS (100%).

...
- **Severity:** Medium
- **Area:** QA / Tooling
- **Status:** FIXED — Reproducible repository automation and GitHub-hosted CI verified
- **Fix Summary:**
  ...
  - Created repository-owned server provisioning tool `scripts/setup_test_server.py` to acquire and prepare `samp03svr` with SHA-256 integrity checks and path traversal protection without committing external binaries.
...
- **Verification:** Verified live on GitHub Actions hosted runner (Run ID `34941878535`, commit `8ec9ed1`, `ubuntu-24.04`). Layer A verified (AP1–AP7 7/7 PASS, RC1–RC5 5/5 PASS). Layer B verified (ELF32 Intel 80386, 6 canonical exports present, 18/18 Pawn compilation with 0 errors and 0 emitted warnings under pinned CI warning policy `-w239`). Layer C verified on headless SA-MP 0.3.7-R2 server with 151 / 151 PASS across all 11 permanent suites (100% assertion coverage, zero regressions). SDK submodule pin verified (`a5ce36a9b6ebbea6ad36705603f653bf3d4f41c5`). Disposable runtime cleanup verified.
...
- **Planned phase:** Phase 14 / 14.1 / 14.2
...
- **Fix Summary:**
  ...
  - On `cbCreate` failure (AMX error) or post-create accounting commit failure (`TryAddActiveTextDrawCount` rejection), SUI refrains from marking `isCreated = true`, debits zero capacity (`activeTextDrawCount` remains uncorrupted), sets `recoveryDestroyRequired = true`, and immediately attempts a best-effort compensating destroy callback (`cbDestroy`).
...
- **Runtime Verification:** Verified in live headless 32-bit Linux SA-MP dedicated server (`samp03svr`) executing permanent test suite 13 `tests/callback_error_recovery/` (scenarios F1 through F14) achieving 14/14 PASS. Cumulative 179 / 179 executed runtime assertions passed across 13 permanent suites.
- **Evidence:** `src/Core.hpp`, `src/Core.cpp`, `tests/callback_error_recovery/`, `tests/RUNTIME_MATRIX.md`.
- **Planned phase:** Phase 17 / 17.1
